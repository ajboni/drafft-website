...
date: 2025-01-13
description: Learn how to use the Director section in Drafft to manage actors and their properties.
---
...
Write a biography or any relevant markdown text in the bio tab.

### Properties

In this tab you can add plain text or structured data in `hjson`, `json` `toml`. This data will be serialized as JSON when generating the game export.

Name, age and gender are automatically created at random when a new actor is detected.

Use the options (gear) menu in the toolbar to toggle **Hide from autocomplete**. When enabled, the actor no longer appears as a suggestion when typing `::` in a script, but it stays available here in the Director section.

:::tip
When you type `::` in a script, Drafft suggests all Actors in your project, with actors already used in that script listed first.
:::

### Expressions

A list of images to use when a script using `UAF` calls an expression `[expression]`. In our example before we added `[happy]` so go ahead and click on `Add Expression` an add a picture for the happy expression.
...
While we are here, you can click on `Screenplay` toolbar button to view/download the current screenplay output. For format details, see [Screenplay Generation](/docs/reference/screenplay_generation).

## Next Step

Continue to the dialogue tree workflow to connect actor content with branching logic and simulation.

> In the meantime, if you need help please visit [the Drafft forums](https://community.drafft.dev)
