...
- `Scenarios` - Variable state diffs (overrides relative to authored defaults)
...
### Get JSON Schema Export

Gets a JSON Schema (Draft 2020-12) for the same payload shape produced by game export.

This endpoint supports the same `collections` query filter as `/game-export`.

```rest
GET /schema-export
GET /schema-export?collections=Scripts,Dialogues,Variables,Scenarios,Scenes
```

**Response:**

- Returns a Draft 2020-12 JSON Schema object.
- Collections with dedicated schema modules (Maps, Variables, Scenarios) produce typed, rule-driven schemas.
- Remaining collections fall back to the `BaseDocument` shape with common fields.
- Variable documents each get their own typed `$defs` entry, with property types inferred from authored default values.
