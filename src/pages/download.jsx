...
import CHANGELOG from "!!raw-loader!@site/shared/CHANGELOG.md";
...
function Download() {
  ...
  const releaseNotes = CHANGELOG.split("## ")
    .map((note) => {
      const [version, ...content] = note.split("\n");
      return {
  ...
  useEffect(() => {
    async function getReleaseinfo() {
      try {
        ...
        // console.log("Release info", releases);

        /* If releases [0].assets is empty, it means there are no assets for this release.
        In this case, we fallback to the most recent release with assets. 
        Also get the index to slice the array.
        */

        if (releases[0].assets.length === 0) {
          setReleaseInfo(releases[1]);
          setPreviousReleases(releases.slice(2, 5));
          return;
        }

        setReleaseInfo(releases[0]);
        setPreviousReleases(releases.slice(1, 5));
      } catch (err) {}
    }
    getReleaseinfo();
  }, []);
        ...
        <div className="bg-dark-background-darker rounded-lg shadow-xl overflow-hidden border border-dark-background-lightest mb-8">
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              {filteredAssets?.map((asset) => (
                <div
                  ...
                  data-umami-event={`download${asset.label}`}
            ...
            {releaseNotes.map((note, index) => (
              <div key={index} className="border border-dark-background-lightest rounded-lg p-4 bg-gray-700/30 mt-2">
                <div className="flex items-center mb-3">
                  <LuFileText className="w-5 h-5 text-primary-color-lighter mr-2" />
                  <div className="text-lg font-medium text-gray-200">{note.version}</div>
                </div>
